...
<head>
    ...
    <title>Login - {{ config('app.name', "Malbi's Kitchen") }}</title>
...
<body class="bg-gradient-to-br from-amber-50 to-orange-50 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
        <div class="text-center mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ config('app.name', "Malbi's Kitchen") }}</h1>
            <p class="text-gray-600">Admin Dashboard</p>
        </div>

        @if(config('admin.show_credentials_on_login'))
            <div class="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                ...
                <dl class="text-sm text-amber-950 space-y-1 font-mono">
                    <div class="flex gap-2"><dt class="text-amber-800 shrink-0">Email</dt><dd class="break-all">{{ config('admin.email') }}</dd></div>
                    <div class="flex gap-2"><dt class="text-amber-800 shrink-0">Password</dt><dd>{{ config('admin.password') }}</dd></div>
                </dl>
        ...
        <form method="POST" action="{{ route('login.post') }}" class="space-y-6">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', config('admin.email')) }}" required autofocus
                        ...
                        placeholder="{{ config('admin.email') }}">
                </div>
