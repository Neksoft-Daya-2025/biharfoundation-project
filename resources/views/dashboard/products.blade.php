...
@section('page-description', 'Manage menu / products')
...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('dashboard.products.create') }}" class="px-4 py-2 bg-[#937237] hover:bg-[#7a5d2e] text-white rounded-lg font-medium transition-colors flex items-center gap-2">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50" data-product-id="{{ $product->id }}">
                        <td class="px-4 py-4 w-12">
                            <input type="checkbox" class="product-checkbox rounded border-gray-300 text-[#937237] focus:ring-[#937237]" value="{{ $product->id }}" aria-label="Select {{ $product->name }}">
                        </td>
                        <td class="px-6 py-4">
                            <span class="font-medium text-gray-900">{{ $product->name }}</span>
                            @if($product->description)
                            <p class="text-xs text-gray-500 mt-0.5">{{ Str::limit($product->description, 50) }}</p>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $product->productCategory?->name ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ format_money($product->price) }}</td>
                        <td class="px-6 py-4 text-sm">{{ $product->is_popular ? 'Yes' : 'No' }}</td>
                        <td class="px-6 py-4 text-sm">{{ $product->is_available ? 'Yes' : 'No' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $product->sort_order }}</td>
                        <td class="px-6 py-4 text-right text-sm space-x-2">
                            <a href="{{ route('dashboard.products.show', $product) }}" class="text-gray-600 hover:text-[#937237] hover:underline">View</a>
                            <a href="{{ route('dashboard.products.edit', $product) }}" class="text-[#937237] hover:underline">Edit</a>
                            <form action="{{ route('dashboard.products.destroy', $product) }}" method="POST" class="inline" onsubmit="return confirm('Delete this product?');">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="px-6 py-8 text-center text-gray-500">No products yet. <a href="{{ route('dashboard.products.create') }}" class="text-[#937237] hover:underline">Add one</a></td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<form id="bulk-delete-form" method="POST" action="{{ route('dashboard.products.bulk-destroy') }}" class="hidden">
    @csrf
...
(function() {
    ...
    window.bulkView = function() {
        ...
        window.location.href = '{{ url("dashboard/products") }}/' + ids[0];
    };
    window.bulkEdit = function() {
        const ids = getSelectedIds();
        if (ids.length === 0) return;
        window.location.href = '{{ url("dashboard/products") }}/' + ids[0] + '/edit';
    };
