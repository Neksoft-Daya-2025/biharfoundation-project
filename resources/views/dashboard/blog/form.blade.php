...
@section('page-description', $post ? 'Update blog post' : 'Create a new blog post')

@section('content')
<div class="max-w-4xl">
    <div class="mb-6">
        <a href="{{ route('dashboard.blog') }}" class="text-[#937237] hover:underline flex items-center gap-2">
            <i data-feather="arrow-left" class="w-4 h-4"></i> Back to Blog
        </a>
    </div>

    <form action="{{ $post ? route('dashboard.blog.update', $post) : route('dashboard.blog.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" id="slug" name="slug" value="{{ old('slug', $post->slug ?? '') }}" placeholder="Auto-generated from title if empty"
        ...
        <div>
            ...
            <textarea id="excerpt" name="excerpt" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#937237] focus:border-[#937237] outline-none @error('excerpt') border-red-500 @enderror">{{ old('excerpt', $post->excerpt ?? '') }}</textarea>
            @error('excerpt')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>

        <div>
            <label for="content" class="block text-sm font-medium text-gray-700 mb-1">Content *</label>
            <textarea id="content" name="content" rows="12" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#937237] focus:border-[#937237] outline-none font-mono text-sm @error('content') border-red-500 @enderror">{{ old('content', $post->content ?? '') }}</textarea>
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-4 items-start">
                <div class="space-y-2">
                    ...
                    <input type="text" id="featured_image" name="featured_image" value="{{ old('featured_image', ($post && $post->featured_image && str_starts_with($post->featured_image, 'http')) ? $post->featured_image : '') }}" placeholder="https://..."
                ...
                @php $imgUrl = $post ? $post->featured_image_url : null; @endphp
                @if($imgUrl)
                <div class="flex-shrink-0">
                    <img id="featured_image_preview" src="{{ $imgUrl }}" alt="Featured" class="w-32 h-32 object-cover rounded-lg border border-gray-200">
                </div>
                @else
            ...
            @error('featured_image_file')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>
        ...
        <div class="border-t border-gray-200 pt-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="meta_title" name="meta_title" value="{{ old('meta_title', $post->meta_title ?? '') }}" maxlength="255" placeholder="Used in &lt;title&gt; and social"
                ...
                <div>
                    ...
                    <textarea id="meta_description" name="meta_description" rows="2" maxlength="500" placeholder="Short description for search and social"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#937237] focus:border-[#937237] outline-none">{{ old('meta_description', $post->meta_description ?? '') }}</textarea>
                </div>
                <div>
                    <label for="meta_keywords" class="block text-sm font-medium text-gray-700 mb-1">Meta keywords</label>
                    <input type="text" id="meta_keywords" name="meta_keywords" value="{{ old('meta_keywords', $post->meta_keywords ?? '') }}" maxlength="255" placeholder="Comma-separated keywords"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#937237] focus:border-[#937237] outline-none">
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-3 pt-4">
            ...
            <a href="{{ route('dashboard.blog') }}" class="px-6 py-2 border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-50 transition-colors">Cancel</a>
        </div>
    </form>
</div>
